...
class Typecho_Widget_Helper_Form_Element_Checkbox extends Typecho_Widget_Helper_Form_Element
{
    ...
    {
        $checkboxContainer = new Typecho_Widget_Helper_Layout('div', array('class' => 'col-md-12'));
        foreach ($options as $value => $label) {
            $this->_options[$value] = new Typecho_Widget_Helper_Layout('input', array(
              'class' => 'form-check-input'
            ));

            $item = $this->multiline();
            $item->setAttribute('class', 'form-check');
            $checkboxContainer->addItem($item);
            ...
            $labelItem = new Typecho_Widget_Helper_Layout('label', array('class' => 'form-check-label'));
